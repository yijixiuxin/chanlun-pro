...
from chanlun.trader.trader_qmt_option import QMTTraderOption

"""
QMT 实盘/模拟交易运行脚本 - 期权(Option)
"""

logger = fun.get_logger("trader_qmt_option.log")

# ================= 配置区域 =================

# 1. 定义监控的代码列表
# 期权示例: 具体合约代码 (项目标准格式 Exchange.Code)
run_codes = ['CFFEX.MO2603-C-8400'] 
market_type = 'option' # 期权市场类型
...
def run_trader():
    ...
    # 初始化交易对象
    TR = QMTTraderOption(name="QMT_Option_Trader", log=logger.info)

    # 初始化行情数据对象
    Data = OnlineMarketDatas(market_type, frequencys, ex, cl_config)

    # 加载历史数据 (如果有保存)
    TR.load_from_pkl("trader_qmt_option")
    ...
    while True:
        try:
            ...
            # 保存交易状态
            TR.save_to_pkl("trader_qmt_option")
