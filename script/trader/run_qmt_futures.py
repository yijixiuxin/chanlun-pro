...
from chanlun.trader.trader_qmt_futures import QMTTraderFutures

"""
QMT 实盘/模拟交易运行脚本 - 期货(Futures)
"""

logger = fun.get_logger("trader_qmt_futures.log")

# ================= 配置区域 =================

# 1. 定义监控的代码列表
# 期货示例: 使用项目标准格式 (Exchange.Code)
run_codes = ['SHFE.ag2306', 'CFFEX.IF2306']
market_type = 'futures'
...
def run_trader():
    ...
    # 初始化交易对象
    TR = QMTTraderFutures(name="QMT_Futures_Trader", log=logger.info)

    # 初始化行情数据对象
    Data = OnlineMarketDatas(market_type, frequencys, ex, cl_config)

    # 加载历史数据 (如果有保存)
    TR.load_from_pkl("trader_qmt_futures")
    ...
    while True:
        try:
            ...
            # 保存交易状态
            TR.save_to_pkl("trader_qmt_futures")
