...
class StrategyAXDTradeModel(Strategy):
    ...
    def on_bt_loop_start(self, bt: BackTest):
        ...
        for code in bt.codes:
            ...
            if len(cd.get_xds()) > 0:
                ...
                xd = cd.get_xds()[-1]
                if xd.type == 'down' and (xd.mmd_exists(['1buy', '2buy', '3buy']) or xd.bc_exists(['pz'])):
    ...
    def open(self, code, market_data: MarketDatas, poss: Dict[str, POSITION]) -> List[Operation]:
        ...
        stop_loss_price = self.get_max_loss_price('buy', price, stop_loss_price, self._max_loss_rate)

        # 低级别线段走势段要有配合的背驰或买卖点
        xds_down_30m = [_xd for _xd in cd_30m.get_xds() if
                        _xd.start.k.date >= xd_day.start.k.date and _xd.type == 'down']
        zsds_down_30m = [_zsd for _zsd in cd_30m.get_zsds() if
                         _zsd.start.k.date >= xd_day.start.k.date and _zsd.type == 'down']
        xds_down_5m = [_xd for _xd in cd_5m.get_xds() if _xd.start.k.date >= xd_day.start.k.date and _xd.type == 'down']
        zsds_down_5m = [_zsd for _zsd in cd_5m.get_zsds() if
                        _zsd.start.k.date >= xd_day.start.k.date and _zsd.type == 'down']

        score_val = 0
        for _xd in xds_down_30m[-2:]:
            if _xd.mmd_exists(['1buy', '2buy', '3buy']) or _xd.bc_exists(['xd', 'pz', 'qs']):
                score_val += 1
                break
        for _zsd in zsds_down_30m[-2:]:
            if _zsd.mmd_exists(['1buy', '2buy', '3buy']) or _zsd.bc_exists(['zsd', 'pz', 'qs']):
                score_val += 1
        ...
        info = {
            'day_zsd_type': 0 if len(cd_day.get_zsds()) == 0 else cd_day.get_zsds()[-1].type,
        ...
        for bc in xd_day.get_bcs():
            ...
            return [
                Operation(
                    opt='buy', mmd=bc_mmd, loss_price=stop_loss_price, info=info,
    ...
    def close(self, code, mmd: str, pos: POSITION, market_data: MarketDatas) -> [Operation, None, List[Operation]]:
        ...
        if bi_day.type == 'up' and bi_day.is_done() and last_day_date > bi_day.end.klines[-1].klines[-1].date:
        ...
        # 30M 线段 至少做3段
        xds_30m = [_xd for _xd in cd_30m.get_xds() if _xd.start.k.date >= info['bi_end_date']]
        if len(xds_30m) >= 3:
        ...
        # 5m 出现三类卖点，退出
        bi_day = cd_day.get_bis()[-1]
        xds_5m = [_xd for _xd in cd_5m.get_xds() if _xd.start.k.date >= info['bi_end_date']]
        if len(xds_5m) >= 5 and bi_day.type == 'up' and bi_day.is_done() and price < bi_day.end.klines[-1].l:
            bis_5m = [
                _bi for _bi in cd_5m.get_bis()
                if _bi.start.k.date >= bi_day.end.klines[0].klines[0].date and _bi.type == 'up'
            ]
            for _bi in bis_5m:
