...
def identify_fractals(cl_klines: List[CLKline]) -> List[FX]:
    ...
    规则:
    1. 顶分型: 中间K线的最高点和最低点都是三根K线中最高的。
    2. 底分型: 中间K线的最高点和最低点都是三根K线中最低的。
    3. 不允许K线重叠: 识别出一个分型后，下一次识别将从该分型之后开始。
    """
    final_fxs: List[FX] = []
    ...
    # 使用while循环，方便在找到分型后跳过K线
    i = 1
    while i < len(cl_klines) - 1:
        prev_k = cl_klines[i - 1]
        curr_k = cl_klines[i]
        next_k = cl_klines[i + 1]

        # 检查是否构成顶分型
        is_ding = curr_k.h > prev_k.h and curr_k.h > next_k.h and \
                  curr_k.l > prev_k.l and curr_k.l > next_k.l

        # 检查是否构成底分型
        is_di = curr_k.l < prev_k.l and curr_k.l < next_k.l and \
                curr_k.h < prev_k.h and curr_k.h < next_k.h

        if is_ding:
            fx = FX(_type='ding', k=curr_k, klines=[prev_k, curr_k, next_k], val=curr_k.h)
            final_fxs.append(fx)
            # 找到一个分型后，跳过构成此分型的K线，防止重叠
            i += 4
        elif is_di:
            fx = FX(_type='di', k=curr_k, klines=[prev_k, curr_k, next_k], val=curr_k.l)
            final_fxs.append(fx)
            # 找到一个分型后，跳过构成此分型的K线，防止重叠
            i += 4
        else:
            # 如果当前三根K线不构成任何分型，则向后移动一根K线继续检查
            i += 1
