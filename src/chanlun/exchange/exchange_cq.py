...
class ExchangeChangQiao(Exchange):
    ...
    def all_stocks(self):
        ...
        if self.stock_list_cache is not None:
            LogUtil.info("Returning from cache...")
    ...
    def now_trading(self):
        ...
        """
        if pytz is None:
            LogUtil.info("Error: pytz library is required for accurate timezone conversion. Cannot check trading time.")
            return False
                    ...
                    for trade_info in session.trade_sessions:
                        ...
                        try:
                            ...
                            if not isinstance(start_time, time) or not isinstance(end_time, time):
                                LogUtil.info(f"Warning: Skipping trade session, attributes are not datetime.time objects.")
                                continue
                            ...
                            LogUtil.info(f"Warning: TradeSessionInfo object structure unexpected. Skipping.")
                            continue
                    return False

            LogUtil.info("Warning: Market.US not found in trading sessions response.")
            return False
