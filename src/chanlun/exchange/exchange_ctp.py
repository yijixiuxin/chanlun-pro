...
class MarketCTP(Exchange):
    def __init__(self):
        ...
        # CTP配置，从配置文件读取
        self.broker_id = config.CTP_BROKER_ID if hasattr(config, 'CTP_BROKER_ID') else "9999"
        ...
        self.app_id = config.CTP_APP_ID if hasattr(config, 'CTP_APP_ID') else ""  # 添加AppID
        self.auth_code = config.CTP_AUTH_CODE if hasattr(config, 'CTP_AUTH_CODE') else ""  # 添加授权码
        
        # 行情和交易服务器地址
        self.md_front = config.CTP_MD_FRONT if hasattr(config, 'CTP_MD_FRONT') else "tcp://180.168.146.187:10131"
        self.td_front = config.CTP_TD_FRONT if hasattr(config, 'CTP_TD_FRONT') else "tcp://180.168.146.187:10130"
        ...
        class MdSpi(CThostFtdcMdApi):
            ...
            def OnRspUserLogin(self, pRspUserLogin: CThostFtdcRspUserLoginField, 
                ...
                if pRspInfo and pRspInfo.ErrorID == 0:
                    self.logged_in = True
                    # 检查是否有Level2权限
                    self.has_level2 = hasattr(pRspUserLogin, 'UserLevel') and pRspUserLogin.UserLevel > 0
                    print(f"行情服务器登录成功, {'有' if self.has_level2 else '无'}Level2权限")
                else:
                    print(f"行情服务器登录失败：{pRspInfo.ErrorMsg if pRspInfo else '未知错误'}")
            ...
            def OnRtnDepthMarketData(self, pDepthMarketData: CThostFtdcDepthMarketDataField) -> None:
                ...
                tick_data = {
                    ...
                    'rate': (pDepthMarketData.LastPrice - pDepthMarketData.PreClosePrice) / pDepthMarketData.PreClosePrice * 100 if pDepthMarketData.PreClosePrice != 0 else 0
                }
                ...
                if self.has_level2 and hasattr(pDepthMarketData, 'BidPrice20'):
                    for i in range(1, 21):
                        ...
                        tick_data[f'buy{i}_volume'] = getattr(pDepthMarketData, f'BidVolume{i}')
                        tick_data[f'sell{i}'] = getattr(pDepthMarketData, f'AskPrice{i}')
                        tick_data[f'sell{i}_volume'] = getattr(pDepthMarketData, f'AskVolume{i}')
                # 普通五档行情
                else:
                    for i in range(1, 6):
                        tick_data[f'buy{i}'] = getattr(pDepthMarketData, f'BidPrice{i}')
    ...
    def klines(self, code: str, frequency: str, start_date: str = None, end_date: str = None, args=None) -> Union[pd.DataFrame, None]:
        """获取K线数据"""
