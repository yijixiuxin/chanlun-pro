...
def trader_view(request):
    ...
    currency_result = trader.traders_result([currency_trader], lambda a:a).get_html_string(attributes={'class': 'table'})

    # 港股交易对象
    hk_stock_trader = pickle.loads(rd.get_byte('trader_hk_stock'))

    hk_stock_result = trader.traders_result([hk_stock_trader], lambda a: a).get_html_string(attributes={'class': 'table'})
