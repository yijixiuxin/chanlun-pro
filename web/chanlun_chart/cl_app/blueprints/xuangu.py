...
def xuangu_task_list(market):
    ...
    task_infos = {
        _k: {"task_memo": _v["task_memo"], "frequency_memo": _v["frequency_memo"]}
...
def xuangu_task_add():
    ...
    frequencys = request.form["frequencys"].split(",")
    zx_group = request.form["zx_group"]
    opt_type = request.form["opt_type"].split(",")

    if task_name not in _xuangu_tasks.xuangu_task_config_list().keys():
        return {"ok": False, "msg": "选股任务不存在"}

    allow_freq_num = _xuangu_tasks.xuangu_task_config_list()[task_name]["frequency_num"]
    if len(frequencys) != allow_freq_num:
        return {"ok": False, "msg": f"选股周期错误，该任务可选周期数量 : {allow_freq_num}"}

    run_res = _xuangu_tasks.run_xuangu(market, task_name, frequencys, opt_type, zx_group)
