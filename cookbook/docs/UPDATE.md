...
* 新增前端图表止损线设置，后台检查并发送飞书警报的功能；在前端TV图表使用水平线设置止损线，文本为“止损线”/“做空止损线”，后台会每300秒检查，触发则报警。
...
> config.py 中统一AI模型配置（老版本需要重新配置）

* 新增 AI 模型配置（增加更多平台模型，只需要支持 OpenAI 格式调用即可）
* 图表中增加 AI 后续趋势预测（需要配置 AI 模型，目前还比较初级，有很大优化空间）
